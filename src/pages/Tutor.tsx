...
// CHỈ ĐỔI ĐÚNG DÒNG NÀY: Dùng hàm Stream để nhả chữ nhanh
...
export default function Tutor({ user }: { user: any }) {
  ...
  // CHỈ SỬA LOGIC TRONG HÀM NÀY ĐỂ AI GÕ CHỮ TỪNG DÒNG
  const handleSend = async () => {
    ...
    const aiPlaceholder: Message = { role: 'model', parts: [{ text: '' }] }; // Tạo bong bóng chat trống cho AI
    
    // Đẩy ngay câu hỏi của user và bong bóng trống của AI lên màn hình
    setMessages(prev => [...prev, userMessage, aiPlaceholder]);
    
    const currentInput = input;
    const currentImage = imageBase64?.split(',')[1];
    const history = messages; // Lưu lịch sử chat
    
    setInput(''); setImageBase64(null); setLoading(true);
    
    try {
      // Gọi hàm Stream: Chữ từ API trả về đến đâu, nhét vào bong bóng chat đến đó
      await getTutorStream(currentInput, history, currentImage, (chunk) => {
        setMessages(prev => {
          const newMessages = [...prev];
          const lastMsg = newMessages[newMessages.length - 1];
          if (lastMsg.role === 'model') {
            lastMsg.parts[0].text += chunk; // Cộng dồn chữ vào
          }
          return newMessages;
        });
        setLoading(false); // Tắt hiệu ứng nhấp nháy ngay khi có chữ đầu tiên
      });
    } catch (error) { 
      console.error(error); 
      setMessages(prev => {
        ...
        if (lastMsg.role === 'model' && !lastMsg.parts[0].text) {
          lastMsg.parts[0].text = "Xin lỗi Linh, đang bị lỗi mạng. Thử lại nhé!";
        }
        return newMessages;
      });
    } finally { 
  ...
  // TỪ ĐÂY TRỞ XUỐNG LÀ GIAO DIỆN XỊN XÒ CỦA LINH, GIỮ NGUYÊN 100%
  return (
