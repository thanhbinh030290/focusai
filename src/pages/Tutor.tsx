...
import { motion, AnimatePresence } from 'motion/react'; // Dùng đúng motion/react để tránh lỗi trắng màn hình
import { Send, Bot, User, Sparkles, BookOpen, GraduationCap, Image as ImageIcon, X, Zap } from 'lucide-react';
import Markdown from 'react-markdown';
import { getTutorStream } from '../services/geminiService'; // Đổi sang hàm Stream
...
export default function Tutor({ user }: { user: any }) {
  ...
  const handleSend = async () => {
    ...
    const userMessage: Message = { 
      role: 'user', 
      parts: [{ text: input || "Phân tích ảnh này giúp mình." }] 
    };

    // Tạo tin nhắn trống cho AI để chuẩn bị nhận chữ đổ vào
    ...
    try {
      // GỌI HÀM STREAM: Chữ về đến đâu cập nhật đến đó
      await getTutorStream(currentInput, history, currentImage, (chunk) => {
        ...
        setLoading(false); // Tắt loading ngay khi có chữ đầu tiên về
      });
    } catch (error) {
      console.error(error);
      setMessages(prev => [...prev, { role: 'model', parts: [{ text: "Kết nối hơi lag, Linh thử lại nhé!" }] }]);
    } finally {
      setLoading(false);
    }
  };
        ...
        <div ref={scrollRef} className="flex-1 overflow-y-auto p-10 space-y-8 scrollbar-hide">
          {messages.length === 0 && (
            <div className="h-full flex flex-col items-center justify-center text-center max-w-lg mx-auto space-y-8 py-12">
              ...
              <h3 className="text-3xl font-black text-text-main">Chào {firstName}! 👋 Mình đây.</h3>
              <p className="text-text-muted text-xl font-bold italic leading-relaxed">Gửi câu hỏi hoặc bài tập khó cho mình nhé!</p>
              <div className="grid grid-cols-2 gap-6 w-full pt-6">
                <div onClick={() => setInput("Giải bài tập Toán lớp 12 này...")} className="p-6 bg-white rounded-3xl border-2 border-violet-50 text-left hover:border-violet-300 transition-all cursor-pointer shadow-sm group">
                ...
                <div onClick={() => setInput("Tóm tắt giúp mình kiến thức chương...")} className="p-6 bg-white rounded-3xl border-2 border-violet-50 text-left hover:border-violet-300 transition-all cursor-pointer shadow-sm group">
                  <Zap className="text-yellow-500 mb-3 group-hover:scale-110 transition-transform" size={32} />
                  <p className="font-black text-text-main">Tóm tắt bài</p>
                </div>
              </div>
            </div>
          )}
        ...
        <div className="p-8 bg-white border-t border-violet-100 backdrop-blur-md">
          ...
          <div className="relative flex items-center gap-3 bg-violet-50 p-2 rounded-[30px] border-2 border-violet-100 focus-within:border-violet-400 transition-all shadow-inner">
            ...
            <textarea 
              ...
              placeholder={`Hỏi ${firstName} bất cứ điều gì về bài tập...`} 
