...
const MOCK_FRIENDS = [
  ...
  { id: 103, name: 'Bá Khánh', level: 7, points: 2100 },
];

const MOCK_SUGGESTIONS = [
  { id: 201, name: 'Minh Thư', level: 2, points: 300 },
  { id: 202, name: 'Hoàng Hải', level: 4, points: 900 },
];

const MOCK_INVENTORY = [
  { type: 'character', item_id: 1 },
  { type: 'character', item_id: 3 },
  { type: 'vehicle', item_id: 1 },
  { type: 'vehicle', item_id: 2 },
];
// ---------------------------------------------
...
export default function Profile({ user, setUser }: { user: any, setUser: (user: any) => void }) {
  ...
  const handleSearch = (q: string) => {
    ...
    const results = [...MOCK_FRIENDS, ...MOCK_SUGGESTIONS].filter(u => 
      u.name.toLowerCase().includes(q.toLowerCase())
    );
    setSearchResults(results);
  };

  const viewPublicProfile = (id: number) => {
  ...
  const handleRename = () => {
    ...
    if ((user?.points || 0) < cost) {
      alert('Không đủ điểm để đổi tên!');
      return;
    }
    setUser({ ...user, name: newName, points: user.points - cost, rename_count: (user.rename_count || 0) + 1 });
    setIsRenaming(false);
  };
  ...
  const selectedChar = CHARACTERS.find(c => c.id === (user?.selected_character_id || 1));
          ...
          <div className="flex flex-col md:flex-row items-center md:items-end gap-6 md:gap-8 -mt-20 md:-mt-24">
            ...
            <div className="flex-1 text-center md:text-left mb-2 md:mb-4">
              <div className="flex flex-col md:flex-row md:items-center gap-3">
                <h1 className="text-4xl md:text-5xl font-black text-text-main tracking-tight">{user?.name || 'Người dùng'}</h1>
        ...
        <div className="xl:col-span-2 space-y-8">
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
            ...
            <div className="bg-gradient-to-br from-fuchsia-500 to-purple-600 p-6 rounded-[30px] shadow-xl text-white relative overflow-hidden group">
              ...
              <p className="text-4xl font-black">{inventory?.length || 0}</p>
            </div>
          </div>
